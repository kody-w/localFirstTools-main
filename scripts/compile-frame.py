#!/usr/bin/env python3
"""
compile-frame.py -- Deterministic frame compiler for Moltbook.

Reads an HTML post, extracts moltbook:generation and moltbook:seed meta tags,
...
def deterministic_compile(html, generation, seed):
    ...
    # Always increment moltbook:generation
    html = set_meta(html, "moltbook:generation", str(generation))
    ...
    compile_comment = f"<!-- moltbook:compiled gen={generation} focus={focus_name} id={compile_id} -->"
...
def _apply_performance(html, rng):
    ...
    perf_comment = "<!-- moltbook:perf-hint Use requestAnimationFrame for animations -->"
    if perf_comment not in html and "<script>" in html.lower():
        html = re.sub(
            r"(<script[^>]*>)",
            f"\\1\n// moltbook:perf — prefer requestAnimationFrame over setInterval for animation loops",
            html,
            count=1,
            flags=re.IGNORECASE,
        )
...
def _apply_polish(html, rng):
    ...
    polish_hint = "// moltbook:polish — wrap risky operations in try/catch"
...
def _apply_refinement(html, rng):
    ...
    refine_hint = "<!-- moltbook:refinement-pass Complete -->"
...
def copilot_compile(html, filename, generation, seed):
    ...
    prompt = f"""You are a deterministic HTML frame compiler for Moltbook.
...
7. The <meta name="moltbook:generation" content="{generation}"> tag MUST be present with value {generation}
8. The <meta name="moltbook:seed" content="{seed}"> tag MUST be preserved unchanged
...
def compile_frame(file_path, dry_run=False, no_llm=False, verbose=False):
    ...
    # Extract moltbook meta tags
    gen_str = extract_meta(html, "moltbook:generation")
    current_gen = int(gen_str) if gen_str is not None else 0

    seed_str = extract_meta(html, "moltbook:seed")
    ...
    if not no_llm:
        ...
        if backend == "copilot-cli":
            ...
            if output_html:
                ...
                llm_gen = extract_meta(output_html, "moltbook:generation")
                if llm_gen != str(next_gen):
                    output_html = set_meta(output_html, "moltbook:generation", str(next_gen))
                if verbose:
...
def main():
    parser = argparse.ArgumentParser(
        description="Deterministic frame compiler for Moltbook. "
